...
| 🧭 **结尾自动简报 Auto briefing** | 底部状态栏每轮回复后刷新一行简报。A statusLine refreshes a one-line summary after every reply. |
...
另外，**底部状态栏**每轮回复后会自动刷新一行紧凑简报（`xteam 5会话·0锁·聊0 | 我无锁`）。*Also: the bottom status line auto-refreshes a one-line summary after every reply.*
